...
type TakeToolModalComponentProps = {
  isTakeToolModalVisible: boolean;
  setIsTakeToolModalVisible: Dispatch<SetStateAction<boolean>>;
  tool: ToolProps;
  navigation: any;
};

export default function TakeConsmableModalComponent({
  isTakeToolModalVisible,
  setIsTakeToolModalVisible,
  tool,
  navigation,
}: TakeToolModalComponentProps) {
  ...
  const onLayout = () => {
    ...
    console.log(projectTopValue);
    console.log(ownerTopValue);
  };
  ...
  return (
    <Modal visible={isTakeToolModalVisible} transparent={true}>
      <View style={styles.container}>
        <View style={styles.main} ref={mainViewnRef}>
          <Text style={styles.headText}>{"Взяти інструмент"}</Text>

          <PermissionCheck
            permissionsToBeVisible={[27]}
            elseChildren={
              <TouchableOpacity
                activeOpacity={1}
                ref={ownerButtonRef}
          ...
          <View style={styles.buttonView} onLayout={onLayout}>
            <TouchableOpacity
              ...
              onPress={() => setIsTakeToolModalVisible(false)}
            >
